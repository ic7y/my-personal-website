...
> 前言：协同过滤算法是众多推荐算法之一，但以其优秀的算法思想和不复杂的工程实现难度，以及不错的推荐效果，深受各大公司网站所使用。
而spark mllib里的实现更大为降低了该算法的使用，且依赖于spark的大数据处理能力，使得模型很快能够得到验证。
Spark mllib 该库包含了许多机器学习算法，开发者可以不需要深入了解机器学习算法就能开发出相关程序。
