...
3.  软件模型有6类，分别 功能模型、对象模型、__________、配置性组件模型、________________、抽象模型。
...
11.  对象的概念是系统中用来描述客观事物的一个实体，它是构成系统的一个基本单位。类的概念是具有相同属性和服务的一组________的集合，为这些对象提供了统一的抽象描述。
...
30.  状态图是用来描述_________在任一时刻所处的某一特定状态。
...
53.   大数据的“三V”特征分别是__________、_______ 和____________。
...
-->
10. 简述数据流图（DFD）的基本概念及其在软件设计中的作用。
...
18. 大数据场景下，数据工程与传统软件工程最主要的区别是什么？

19. 在企业中，大数据相关职位有哪些，主要工作职责是什么?

20. 什么是数据清洗？数据清洗主要处理哪些脏数据问题？
...
1.  某汽车公司欲开发一套汽车信息管理系统，根据一下描述选择合适的设计模式进行设计：
...
2.  货运公司承运货物收费标准为20元/吨。为了优惠顾客，对于客户按业务量进行优惠收费标准：
  20吨以下收100%；大于20吨不超过30吨收95%；大于30吨不超过40吨收90%；大于40吨收80%。
  试用判定表给出全部集合，然后用判定树和结构化语言表示d。

3. 某企业要搭建离线大数据数仓平台，需要对接业务数据库、系统日志两类数据源。请运用数据工程思想，设计一条简易数据流水线流程，写出从源头到最终数据应用的完整环节，并说明每个环节作用。
...
4. 短视频平台每天产生海量视频上传、用户点赞、评论、转发数据，需计算实时热门视频排行、离线用户兴趣模型、内容分类统计指标。请设计大数据系统，说明
1）实时和离线各自承担什么任务？
2）实时和离线各自可以使用什么技术实现？

<!-- 6.  某学校拟开发一个图书管理系统，对系统的描述如下：
